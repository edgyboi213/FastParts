...
export const SearchPage: React.FC = () => {
  ...
  useEffect(() => {
    setLoading(true);
    setError(null);
    partsApi.getAll().then(data => {
      // Client side filtering for search
      const filtered = data.filter(p => {
        const name = (p as any).Name || p.name || '';
        const desc = (p as any).Description || p.description || '';
      ...
      <div className="flex flex-col md:flex-row gap-12">
        ...
        <div className="flex-grow">
          <div className="flex justify-between items-end mb-10">
            <div>
              <p className="label-caps mb-2">Результаты поиска для:</p>
          ...
          {sortedParts.length === 0 ? (
            <div className="text-center py-24 bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-200">
              ...
              <p className="font-bold text-slate-400 uppercase tracking-widest text-sm">По вашему запросу ничего не найдено</p>
              <Link to="/" className="mt-6 inline-block text-red-600 font-black uppercase tracking-widest text-xs hover:text-slate-900 transition-colors underline underline-offset-8 decoration-2">Вернуться на главную</Link>
            </div>
          ) : (
